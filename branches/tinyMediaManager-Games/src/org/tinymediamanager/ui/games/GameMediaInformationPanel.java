 ...
 * 
 */
public class GameMediaInformationPanel extends JPanel {
   ...
   *          the model
   */
  public GameMediaInformationPanel(GameSelectionModel model) {
    ...
    setLayout(new FormLayout(new ColumnSpec[] { FormFactory.RELATED_GAP_COLSPEC, FormFactory.DEFAULT_COLSPEC, FormFactory.RELATED_GAP_COLSPEC,
        ...
        ColumnSpec.decode("25px"), FormFactory.RELATED_GAP_COLSPEC, FormFactory.DEFAULT_COLSPEC, FormFactory.RELATED_GAP_COLSPEC,
        ColumnSpec.decode("25px"), FormFactory.RELATED_GAP_COLSPEC, FormFactory.DEFAULT_COLSPEC, FormFactory.RELATED_GAP_COLSPEC,
        ColumnSpec.decode("200px"), }, new RowSpec[] { FormFactory.RELATED_GAP_ROWSPEC, FormFactory.DEFAULT_ROWSPEC,
    ...
    JLabel lblGame = new JLabel(BUNDLE.getString("metatag.game")); //$NON-NLS-1$
    add(lblGame, "6, 4");
    ...
    PropertyChangeListener propertyChangeListener = new PropertyChangeListener() {
      public void propertyChange(PropertyChangeEvent propertyChangeEvent) {
        ...
        if ((source.getClass() == GameSelectionModel.class && "selectedGame".equals(property)) || MEDIA_INFORMATION.equals(property)) {
  ...
  private void fillVideoStreamDetails() {
    List<MediaFile> mediaFiles = gameSelectionModel.getSelectedGame().getMediaFiles(MediaFileType.GAME);
  ...
  private void buildAudioStreamDetails() {
    ...
    List<MediaFile> mediaFiles = gameSelectionModel.getSelectedGame().getMediaFilesContainingAudioStreams();

    for (MediaFile mediaFile : mediaFiles) {
      for (int i = 0; i < mediaFile.getAudioStreams().size(); i++) {
        MediaFileAudioStream audioStream = mediaFile.getAudioStreams().get(i);

        if (mediaFile.getType() == MediaFileType.GAME) {
  ...
  private void buildSubtitleStreamDetails() {
    ...
    List<MediaFile> mediaFiles = gameSelectionModel.getSelectedGame().getMediaFilesContainingSubtitles();
