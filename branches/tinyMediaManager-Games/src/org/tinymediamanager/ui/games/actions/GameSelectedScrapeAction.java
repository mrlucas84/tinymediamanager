...
/**
 * The GameSelectedScrapeAction - scrape seletected game - force best match
 * 
 * @author Manuel Laggner
 */
public class GameSelectedScrapeAction extends AbstractAction {
  private static final long           serialVersionUID = -6006932819900795735L;
  private static final ResourceBundle BUNDLE           = ResourceBundle.getBundle("messages", new UTF8Control()); //$NON-NLS-1$

  public GameSelectedScrapeAction() {
    putValue(NAME, BUNDLE.getString("game.scrape.selected.force")); //$NON-NLS-1$
    putValue(SHORT_DESCRIPTION, BUNDLE.getString("game.scrape.selected.force.desc")); //$NON-NLS-1$
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (selectedGames.size() > 0) {
      GameScrapeMetadataDialog dialog = new GameScrapeMetadataDialog(BUNDLE.getString("game.scrape.selected.force")); //$NON-NLS-1$
      ...
      if (dialog.shouldStartScrape()) {
        // scrape
        TmmSwingWorker scrapeTask = new GameScrapeTask(selectedGames, true, options);
