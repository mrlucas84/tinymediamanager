...
import org.tinymediamanager.ui.gamesets.dialogs.GameSetChooserDialog;

/**
 * @author Manuel Laggner
 * 
 */
public class GameSetSearchAction extends AbstractAction {
  private static final long           serialVersionUID = -2260581786599155278L;
  private static final ResourceBundle BUNDLE           = ResourceBundle.getBundle("messages", new UTF8Control()); //$NON-NLS-1$

  /**
   * Instantiates a new search game set action.
   */
  public GameSetSearchAction(boolean withTitle) {
    if (withTitle) {
      putValue(NAME, BUNDLE.getString("gameset.search")); //$NON-NLS-1$
    }
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    for (GameSet gameSet : selectedGameSets) {
      GameSetChooserDialog chooser = new GameSetChooserDialog(gameSet, selectedGameSets.size() > 1 ? true : false);
