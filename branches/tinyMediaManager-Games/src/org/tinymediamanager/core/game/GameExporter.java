...
/**
 * This class exports a list of games to various formats according to templates.
 * 
...
public class GameExporter {
  ...
  /** The Constant LOGGER. */
  private final static Logger LOGGER             = LoggerFactory.getLogger(GameExporter.class);
  ...
  public static List<ExportTemplate> findTemplates() {
    ...
    for (File dir : templateDirs) {
      ...
      if (typeInConfig.equalsIgnoreCase(TemplateType.GAME.name())) {
        ExportTemplate template = new ExportTemplate();
        template.setName(properties.getProperty("name"));
        // template.setType(TemplateType.GAME); // /// ADE
  ...
  /**
   * exports game list according to template file.
   * 
   * @param games
   *          list of games
   ...
   *           the exception
   */
  public static void export(List<Game> games, String pathToTemplate, String pathToExport) throws Exception {
    LOGGER.info("preparing game export; using " + pathToTemplate);
    ...
    engine.registerNamedRenderer(new GameExporter.NamedDateRenderer());
    engine.registerNamedRenderer(new GameExporter.GameFilenameRenderer());
    ...
    if (fileExtension.equalsIgnoreCase("xml")) {
      listExportFile = new File(exportDir, "gamelist.xml");
    }
    if (fileExtension.equalsIgnoreCase("csv")) {
      listExportFile = new File(exportDir, "gamelist.csv");
    }
    if (listExportFile == null) {
      throw new Exception("error creating gamelist file");
    }

    // create list
    LOGGER.info("generating game list");
    FileUtils.deleteQuietly(listExportFile);

    Map<String, Object> root = new HashMap<String, Object>();
    root.put("games", new ArrayList<Game>(games));

    String output = engine.transform(listTemplate, root);

    FileUtils.writeStringToFile(listExportFile, output, "UTF-8");
    LOGGER.info("game list generated: " + listExportFile.getAbsolutePath());
    ...
    if (StringUtils.isNotBlank(detailTemplateFile)) {
      ...
      File detailsDir = new File(exportDir, "games");
      ...
      for (Game game : games) {
        LOGGER.debug("processing game " + game.getTitle());
        // get preferred game name like set up in game renamer
        File detailsExportFile = new File(detailsDir, GameRenamer.createDestinationForFilename(Globals.settings.getGameSettings()
            .getGameRenamerFilename(), game)
            + "." + fileExtension);
      ...
      LOGGER.info("game detail pages generated: " + exportDir.getAbsolutePath());
    }
  ...
  /**
   * The Class GameFilenameRenderer.
   * 
   * @author Manuel Laggner
   */
  public static class GameFilenameRenderer implements NamedRenderer {
    ...
    public String render(Object o, String pattern, Locale locale) {
      if (o instanceof Game) {
        Game game = (Game) o;
        return GameRenamer.createDestinationForFilename(Globals.settings.getGameSettings().getGameRenamerFilename(), game);
      }
      return null;
    }
