...
/**
 * The GameRemoveAction - to remove all selected games from the database
 * 
 * @author Manuel Laggner
 */
public class GameRemoveAction extends AbstractAction {
  private static final long           serialVersionUID = -984567332370801730L;
  private static final ResourceBundle BUNDLE           = ResourceBundle.getBundle("messages", new UTF8Control()); //$NON-NLS-1$

  public GameRemoveAction() {
    putValue(SMALL_ICON, new ImageIcon(getClass().getResource("/org/tinymediamanager/ui/images/Cross.png")));
    putValue(NAME, BUNDLE.getString("game.remove")); //$NON-NLS-1$
  }
  ...
  public void actionPerformed(ActionEvent arg0) {
    ...
    if (selectedGames.size() > 0) {
      for (int i = 0; i < selectedGames.size(); i++) {
        GameList.getInstance().removeGame(selectedGames.get(i));
      }
    }
  }
}
